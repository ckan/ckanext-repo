...
class RepoInfo(p.SingletonPlugin):
    ...
    def _render_repos_info(self):
        return p.toolkit.render_snippet(
            "repo_snippet.html", {"repos_info": self.repos_info}
        )
...
def _get_repo_info(repo_ref, src_path=False):
    ...
    try:
        ...
        repo_info["url"] = "https://github.com/{0}/{1}/commit/{2}".format(
            github_org, repo_name, repo_info["hash"]
        )
    ...
    try:
        ...
        repo_info["last_updated"] = datetime.datetime.fromtimestamp(int(date)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    except subprocess.CalledProcessError:
        pass
